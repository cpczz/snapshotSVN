...
select CASE WHEN versionprueba is null
            THEN 
               CASE WHEN ProgramaServidorWL11PROD is not null
                    THEN 'svn export --username UsuarioSubversion --password PasswordUsuarioSubversion --revision '||versiondesarrollocorte||' ' ||cod_repositorio||RamaDesarrollo||' FuentesBatch-TEST'
               END             
